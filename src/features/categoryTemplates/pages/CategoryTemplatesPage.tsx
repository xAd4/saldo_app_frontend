...
import { setSelectedTemplate, removeTemplate } from "../slices/categoryTemplateSlice";

export default function CategoryTemplatesPage() {
  const dispatch = useAppDispatch();
  const { templates, isLoading } = useAppSelector((state) => state.categoryTemplates);
  ...
  const handleDelete = (id: number) => { dispatch(removeTemplate(id)); };
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl backdrop-blur-sm" style={{ backgroundColor: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.06)', padding: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
            <div>
              ...
              <input id="tpl-name" name="name" type="text" value={formData.name} onChange={handleChange} placeholder="Ej: Comida, Transporte..."
            ...
            <div>
              ...
              <input id="default_percentage" name="default_percentage" type="number" step="0.01" min="0" max="1" value={formData.default_percentage} onChange={handleChange} placeholder="Ej: 0.30"
                className="w-full rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 transition-all"
                style={{ padding: '12px 16px', fontSize: 14, backgroundColor: 'rgba(30,41,59,0.5)', border: '1px solid rgba(255,255,255,0.08)' }} />
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
            <button type="submit" className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-medium hover:from-emerald-400 hover:to-cyan-400 transition-all shadow-lg shadow-emerald-500/20"
              style={{ padding: '12px 28px', fontSize: 14 }}>Guardar Plantilla</button>
          </div>
        </form>
      )}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center text-center animate-fade-in-up-delay-1" style={{ padding: '80px 0' }}>
          <div className="rounded-2xl flex items-center justify-center" style={{ width: 72, height: 72, backgroundColor: 'rgba(100,116,139,0.08)', marginBottom: 20 }}>
            <span className="text-slate-500"><DocIcon size={32} /></span>
          </div>
        ...
        <div className="animate-fade-in-up-delay-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {templates.map((template) => (
            <div key={template.id} className="rounded-2xl backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 group"
              style={{ backgroundColor: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.06)', padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div className="rounded-xl flex items-center justify-center" style={{ width: 44, height: 44, backgroundColor: 'rgba(139,92,246,0.12)', color: '#a78bfa' }}>
                  <DocIcon />
                </div>
                <div className="flex opacity-0 group-hover:opacity-100 transition-opacity" style={{ gap: 4 }}>
                  <button onClick={() => dispatch(setSelectedTemplate(template))} className="rounded-lg text-slate-500 hover:text-white hover:bg-white/5 transition-all" style={{ padding: 8 }}>
                    <EditIcon />
                  </button>
                  <button onClick={() => handleDelete(template.id)} className="rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all" style={{ padding: 8 }}>
                    <TrashIcon />
                  </button>
                </div>
              </div>
              ...
              <span className="inline-block rounded-full text-xs font-medium" style={{ marginTop: 10, padding: '3px 10px', backgroundColor: 'rgba(139,92,246,0.12)', color: '#a78bfa' }}>
                {(template.default_percentage * 100).toFixed(0)}% del ingreso
              </span>
            </div>
          ))}
        </div>
