...
export default function SavingsEntriesPage() {
  ...
  const { entries, totalSavings, isLoading } = useAppSelector((state) => state.savingsEntries);
  ...
  const handleDelete = (id: number) => { dispatch(removeEntry(id)); };
  ...
  const BankIcon = ({ size = 22 }: { size?: number }) => (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      {/* Total */}
      <div className="rounded-2xl animate-fade-in-up-delay-1" style={{
        background: 'linear-gradient(135deg, rgba(139,92,246,0.12) 0%, rgba(139,92,246,0.03) 100%)',
        border: '1px solid rgba(139,92,246,0.18)', padding: 28, display: 'flex', alignItems: 'center', gap: 20
      }}>
        <div className="rounded-xl flex items-center justify-center shrink-0" style={{ width: 52, height: 52, backgroundColor: 'rgba(139,92,246,0.15)', color: '#a78bfa' }}>
          <BankIcon size={26} />
        </div>
        <div>
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl backdrop-blur-sm" style={{ backgroundColor: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.06)', padding: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20 }}>
            <div>
              ...
              <input id="saving-amount" name="amount" type="number" step="0.01" value={formData.amount} onChange={handleChange} placeholder="50.00"
            ...
            <div>
              ...
              <input id="saving-desc" name="description" type="text" value={formData.description} onChange={handleChange} placeholder="Ej: Excedente de Enero"
                className="w-full rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 transition-all"
                style={{ padding: '12px 16px', fontSize: 14, backgroundColor: 'rgba(30,41,59,0.5)', border: '1px solid rgba(255,255,255,0.08)' }} />
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
            <button type="submit" className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-medium hover:from-emerald-400 hover:to-cyan-400 transition-all shadow-lg shadow-emerald-500/20"
              style={{ padding: '12px 28px', fontSize: 14 }}>Guardar Ahorro</button>
          </div>
        </form>
      )}
      ...
      {isLoading ? (
        <div className="flex items-center justify-center" style={{ padding: '80px 0' }}>
          <div className="w-10 h-10 rounded-full animate-spin" style={{ border: '2px solid rgba(139,92,246,0.2)', borderTopColor: '#a78bfa' }} />
        </div>
      ) : entries.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center animate-fade-in-up-delay-2" style={{ padding: '80px 0' }}>
          <div className="rounded-2xl flex items-center justify-center" style={{ width: 72, height: 72, backgroundColor: 'rgba(100,116,139,0.08)', marginBottom: 20 }}>
            <span className="text-slate-500"><BankIcon size={32} /></span>
          </div>
        ...
        <div className="animate-fade-in-up-delay-2" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {entries.map((entry) => (
            <div key={entry.id} className="rounded-2xl backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 group"
              ...
              <div className="flex items-center" style={{ gap: 16 }}>
                <div className="rounded-xl flex items-center justify-center shrink-0" style={{ width: 44, height: 44, backgroundColor: 'rgba(139,92,246,0.12)', color: '#a78bfa' }}>
                  <BankIcon />
                </div>
                <div>
                  ...
                  <p className="text-slate-500" style={{ fontSize: 12, marginTop: 3 }}>
                    ...
                    {entry.monthly_budget_id && ` · Presupuesto #${entry.monthly_budget_id}`}
                  </p>
                </div>
              </div>
              <div className="flex items-center" style={{ gap: 12 }}>
                <span className="font-bold" style={{ fontSize: 18, color: '#a78bfa' }}>${entry.amount.toFixed(2)}</span>
                <button onClick={() => handleDelete(entry.id)} className="rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100" style={{ padding: 8 }}>
                  <TrashIcon />
                </button>
              </div>
            </div>
