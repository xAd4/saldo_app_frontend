...
export default function MonthlyBudgetsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.month || !formData.year || !formData.total_planned_income)
      return;

    setIsSaving(true);
    try {
      await startSavingMonthlyBudget({
        month: parseInt(formData.month, 10),
        year: parseInt(formData.year, 10),
        total_planned_income: parseFloat(formData.total_planned_income),
      });
      setShowForm(false);
      setFormData({ month: "", year: "", total_planned_income: "" });
      await startLoadingMonthlyBudgets();
    } catch {
  ...
  const handleDelete = async (budget: (typeof budgets)[0]) => {
    const result = await SwalCustom.confirm(
      "¿Eliminar presupuesto?",
      `Se eliminará el presupuesto de ${monthNames[budget.month]} ${budget.year} permanentemente.`,
    );

    if (result.isConfirmed) {
      try {
        await startDeletingMonthlyBudget(budget);
        await startLoadingMonthlyBudgets();
      } catch {
        // Error already handled by the hook
      }
      ...
      {showForm && (
        ...
        >
          ...
          >
            ...
            >
              {isSaving ? "Creando..." : "Crear Presupuesto"}
            </button>
          </div>
        </form>
      )}
          ...
          {budgets.map((budget) => (
            ...
            >
              ...
              <p
                ...
                style={{ fontSize: 18, color: "#34d399", marginTop: 8 }}
              >
