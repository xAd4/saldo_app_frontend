...
import { useBudgetCategoriesStore } from "../hooks/useBudgetCategoriesStore";
...
export default function BudgetCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.monthly_budget_id) return;

    setIsSaving(true);
    try {
      await startSavingBudgetCategory({
        monthly_budget_id: parseInt(formData.monthly_budget_id, 10),
        name: formData.name,
        target_percentage: parseFloat(formData.target_percentage) || 0,
      ...
      await startLoadingBudgetCategories();
    } catch {
      // Error already handled by the hook via SwalCustom
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (cat: (typeof categories)[0]) => {
    const result = await SwalCustom.confirm(
      "¿Eliminar categoría?",
      `Se eliminará la categoría "${cat.name}" permanentemente.`,
    );

    if (result.isConfirmed) {
      try {
        await startDeletingBudgetCategory(cat);
        await startLoadingBudgetCategories();
      } catch {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
        ...
        >
          ...
          >
            <button
              type="submit"
              disabled={isSaving}
              className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-medium hover:from-emerald-400 hover:to-cyan-400 transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {isLoadingCategories ? (
        ...
        <div
          className="flex flex-col items-center justify-center text-center animate-fade-in-up-delay-1"
          style={{ padding: "80px 0" }}
        >
          <div
          ...
          <p className="text-slate-400 font-medium" style={{ fontSize: 15 }}>
        ...
        >
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="rounded-2xl backdrop-blur-sm transition-all duration-200 hover:-translate-y-1 group"
              style={{
                backgroundColor: "rgba(15,23,42,0.6)",
                border: "1px solid rgba(255,255,255,0.06)",
                padding: 24,
            ...
            >
              ...
              >
                ...
                >
                  {(Number(cat.target_percentage) * 100).toFixed(0)}% del ingreso
                </span>
                <span
                  className="font-bold"
              ...
              >
                <div
                  ...
                  style={{
                    height: "100%",
                    width: `${Number(cat.target_percentage) * 100}%`,
